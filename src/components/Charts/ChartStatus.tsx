...
export function ChartStatus() {
  ...
  React.useEffect(() => {
    if (country.length > 0) {
      setLoading(true);
      fetch(
        `${process.env.REACT_APP_API_URL}/api/teams/${associateDetails.associateId}/status?countryCode=${country}`,
        {
          method: 'get',
          headers: new Headers({
  ...
  const filteredData = data.filter((eachStatus: any) => eachStatus.value);
